...
def table(var_ind, var_dep, r_value, conclusion, n, resultados=None) -> None:
    ...
    table.add_row(f"n = {n}")
    table.add_section()
    table.add_row(f"Σx = {sum_x:.4f}", f"x̄ = {mean_x:.4f}")
    table.add_section()
    table.add_row(f"Σy = {sum_y:.4f}", f"ȳ = {mean_y:.4f}")
    table.add_section()
    table.add_row(f"Σx² = {sum_x2:.4f}", f"Sₓₓ = {Sxx:.4f}")
    table.add_section()
    table.add_row(f"Σy² = {sum_y2:.4f}", f"Sᵧᵧ = {Syy:.4f}")
    table.add_section()
    table.add_row(f"Σxy = {sum_xy:.4f}", f"Sₓᵧ = {Sxy:.4f}")
    table.add_section()
    console.print(Align(table, align="center"))
    ...
    error_std_b = t_stat_b = t_stat_tabla_b = 0
    ...
    if resultados:
        ...
        t_stat_b = p_beta.get("t_stat_b", 0)
        t_stat_tabla_b = p_beta.get("t_stat_tabla", 0)
    ...
    table.add_row(f"t = {t_stat_b:.4f}")
    table.add_section()
    table.add_row("Valor de tabla")
    table.add_row(f"t_α/2, n - 2 = +- {t_stat_tabla_b:.4f}")
    table.add_section()
    ...
    error_std_r = t_stat_r = t_stat_tabla_r = 0
    ...
    if resultados:
        ...
        t_stat_r = p_rho.get("t_stat_r", 0)
        t_stat_tabla_r = p_rho.get("t_stat_tabla", 0)
    ...
    table.add_section()
    table.add_row("Estadístico de prueba")
    table.add_row(f"t = {t_stat_r:.4f}")
    table.add_section()
    table.add_row("Valor de tabla")
    table.add_row(f"t_α/2, n - 2 = +- {t_stat_tabla_r:.4f}")
    table.add_section()
